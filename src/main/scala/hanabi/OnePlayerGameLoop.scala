...
final class OnePlayerGameLoop[F[_]: Monad: Sync](
  ...
  def start(currentGS: GameState, currentPlayer: Player): F[GameState] = {
    println(currentGS.display(currentPlayer))
    if (currentGS.life.count == 0) currentGS.pure[F]
    else if (currentPlayer.hand.isEmpty && currentGS.hintTokens.count == 0) {
      println(
